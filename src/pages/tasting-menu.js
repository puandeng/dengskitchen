...
async function fetchBrainstorm(courseName, dishName, description) {
  ...
  try {
    ...
    return '⚠️ Could not reach the brainstorm service. Please check your connection and try again.';
  }
}
...
function TagInput({ tags, onChange }) {
  ...
  return (
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, padding: '8px 10px', border: `1px solid ${C.border}`, borderRadius: 8, background: '#fafaf8', minHeight: 42, cursor: 'text' }}>
      {tags.map((t, i) => (
...
function CourseCard({ course, onUpdate, onRemove, onMoveUp, onMoveDown, isFirst, isLast }) {
    ...
    <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, boxShadow: C.shadow, overflow: 'hidden', marginBottom: 20 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 20px', background: C.accentLight, borderBottom: `1px solid ${C.border}` }}>
        ...
        <input
          ...
          style={{ flex: 1, border: 'none', background: 'transparent', fontSize: 17, fontWeight: 700, color: C.text, outline: 'none', fontFamily: 'Georgia, serif' }}
          placeholder="Course name…"
        />
        <div style={{ display: 'flex', gap: 4 }}>
          <button onClick={onMoveUp} disabled={isFirst} title="Move up" style={{ ...btn('#fff', C.muted), padding: '5px 9px', opacity: isFirst ? 0.3 : 1 }}>↑</button>
          <button onClick={onMoveDown} disabled={isLast} title="Move down" style={{ ...btn('#fff', C.muted), padding: '5px 9px', opacity: isLast ? 0.3 : 1 }}>↓</button>
          <button onClick={() => onRemove(course.id)} title="Remove course" style={{ ...btn('#fff', '#c0392b'), padding: '5px 9px' }}>×</button>
        </div>
      </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div>
            ...
            <input
              ...
              style={{ width: '100%', padding: '9px 12px', border: `1px solid ${C.border}`, borderRadius: 8, fontSize: 14, color: C.text, background: '#fafaf8', boxSizing: 'border-box', outline: 'none' }}
            />
          </div>
          <div>
            ...
            <textarea
              ...
              rows={4}
              style={{ width: '100%', padding: '9px 12px', border: `1px solid ${C.border}`, borderRadius: 8, fontSize: 14, color: C.text, background: '#fafaf8', resize: 'vertical', boxSizing: 'border-box', outline: 'none', fontFamily: 'inherit' }}
            />
          </div>
          <div>
            ...
            <TagInput tags={course.ingredients} onChange={tags => update('ingredients', tags)} />
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <input
                ...
                style={{ flex: 1, padding: '9px 12px', border: `1px solid ${C.border}`, borderRadius: 8, fontSize: 13, color: C.text, background: '#fafaf8', outline: 'none' }}
              />
              <button
            ...
            {imgError && (
              <p style={{ marginTop: 8, fontSize: 12, color: '#c0392b' }}>⚠️ Image failed to load. Try generating again.</p>
            )}
          </div>
        </div>
      </div>
      ...
      <div style={{ padding: '0 20px 20px' }}>
        <button
          ...
          style={{ ...btn(showBrainstorm ? C.greenLight : C.green, showBrainstorm ? C.green : '#fff'), border: `1px solid ${C.green}` }}
        >
        ...
        {showBrainstorm && (
          <div style={{ marginTop: 12, padding: 14, background: C.greenLight, border: `1px solid ${C.green}22`, borderRadius: 10 }}>
            {course.brainstormLoading ? (
              <div style={{ color: C.green, fontSize: 13 }}>🍽️ Consulting the chef…</div>
            ) : (
...
function MenuEditor({ roomId, menuName, menuTheme, courses, peers, onUpdateMenuName, onUpdateMenuTheme, onAddCourse, onUpdateCourse, onRemoveCourse, onMoveCourse }) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: C.bg, padding: '28px 20px' }}>
      <div style={{ maxWidth: 960, margin: '0 auto' }}>
        ...
        <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, boxShadow: C.shadow, padding: '28px 32px', marginBottom: 28 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}>
            ...
            <input
              ...
              style={{ flex: 1, border: 'none', background: 'transparent', fontSize: 28, fontWeight: 700, color: C.text, outline: 'none', fontFamily: 'Georgia, serif' }}
        ...
        <button
          onClick={onAddCourse}
          style={{ ...btn(C.accent, '#fff'), width: '100%', justifyContent: 'center', padding: '14px', fontSize: 15, borderRadius: 12, boxShadow: C.shadow }}
        >
          + Add Course
        </button>
      </div>
    ...
    <div style={{ minHeight: '80vh', background: C.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div style={{ maxWidth: 520, width: '100%' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 40 }}>
          ...
          <h1 style={{ fontFamily: 'Georgia, serif', fontSize: 36, color: C.text, margin: '0 0 10px' }}>Menu Atelier</h1>
        ...
        <div style={{ display: 'grid', gap: 16 }}>
          ...
          <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, boxShadow: C.shadow, padding: 28 }}>
            <h2 style={{ fontFamily: 'Georgia, serif', fontSize: 20, color: C.text, margin: '0 0 8px' }}>Create a new menu</h2>
          ...
          {/* Join */}
          <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, boxShadow: C.shadow, padding: 28 }}>
            <h2 style={{ fontFamily: 'Georgia, serif', fontSize: 20, color: C.text, margin: '0 0 8px' }}>Join a session</h2>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <input
                ...
                style={{ flex: 1, padding: '11px 14px', border: `1px solid ${error ? '#e74c3c' : C.border}`, borderRadius: 10, fontSize: 14, color: C.text, outline: 'none', background: '#fafaf8' }}
              />
...
function TastingMenuApp() {
  ...
  const initRoom = useCallback(async (id) => {
    ...
    // After brief delay, initialize if room is empty (new room creator)
    setTimeout(() => {
  ...
  const updateCourse = useCallback((id, updates) => {
    setCourses(prev => {
      ...
      // Don't sync transient UI state keys
      const skipSync = ['brainstormLoading', 'imageLoading'];
...
export default function TastingMenuPage() {
  return (
    ...
    >
      <BrowserOnly fallback={<div style={{ padding: '60px 20px', textAlign: 'center', color: '#7a6655' }}>Loading Menu Atelier…</div>}>
        {() => <TastingMenuApp />}
      </BrowserOnly>
    </Layout>
  );
